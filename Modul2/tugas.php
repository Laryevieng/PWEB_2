...
class Pengumuman {
    ...
    public function add() {
        $query = "INSERT INTO " . $this->table_name . " (judul, isi, tanggal, status, penulis) 
                 VALUES (:judul, :isi, :tanggal, :status, :penulis)";
    ...
    public function toggleStatus() {
        $query = "UPDATE " . $this->table_name . " 
                SET status = CASE WHEN status = 'aktif' THEN 'tidak_aktif' ELSE 'aktif' END 
                WHERE id_pengumuman = :id";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(":id", $this->id_pengumuman);
        return $stmt->execute();
    ...
    public function search($keyword) {
        $query = "SELECT * FROM " . $this->table_name . " 
                WHERE judul LIKE :keyword OR isi LIKE :keyword OR penulis LIKE :keyword 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($_POST['add']) || isset($_POST['update'])) {
        ...
        $stmt = $pengumuman->search($keyword);
    }
}
...
<body>
    <div class="container mt-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div>
                <a href="latihan_02_06.php" class="btn btn-secondary">Kembali ke Daftar Alumni</a>
                <?php if($action == 'list'): ?>
